-- Fix VAT calculation: unit_price is GROSS (incl. VAT), need to calculate NET
...
-- Calculates per-line totals correctly from GROSS input prices
...
SELECT 
  ...
  -- Step 2: Calculate NET from GROSS (remove VAT)
  -- NET = GROSS / (1 + VAT%)
  CASE 
    WHEN oi.currency = o.currency THEN 
  ...
  CASE 
    WHEN oi.currency = o.currency THEN 
      ((oi.unit_price * oi.quantity) / (1 + COALESCE(oi.vat_rate_override, o.vat_rate) / 100)) * COALESCE(oi.discount_percent, 0) / 100
    WHEN oi.currency = 'PLN' AND o.currency = 'EUR' THEN 
      ((oi.unit_price * oi.quantity * COALESCE(oi.fx_rate, 0.3225806451612903)) / (1 + COALESCE(oi.vat_rate_override, o.vat_rate) / 100)) * COALESCE(oi.discount_percent, 0) / 100
    ELSE 
      ((oi.unit_price * oi.quantity) / (1 + COALESCE(oi.vat_rate_override, o.vat_rate) / 100)) * COALESCE(oi.discount_percent, 0) / 100
  ...
  -- Line GROSS (NET + VAT) - should match what client sees
  CASE 
    WHEN oi.currency = o.currency THEN 
